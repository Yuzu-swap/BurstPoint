...
module.exports = {
    ...
    async task(ctx) {
        ...
        do{
            if(startGameBlock == 0){
                await ctx.service.web3.startGame(nowBlock)
                break
            }
            ...
            if(game && game.length == 1){
                if(nowBlock - handleEndBlock > 110){
                    ...
                    await ctx.service.web3.closeGame(handleEndBlock)
                }
                ...
                for(let i = 0; i< addressList.length; i++){
                    ...
                    if(isExist.length == 1){
                        ...
                        await ctx.service.mysql.updateBetInfo(
                            handleEndBlock, addressList[i], ethers.utils.formatUnits(bv, 0), ethers.utils.formatUnits(objectList[i].betAmount,0), success
                        )
                    }
                    else{
